...
import org.processmining.goaldrivenprocessmining.graphHelper.GoalDrivenDot;
import org.processmining.goaldrivenprocessmining.graphHelper.GoalDrivenDotNode;
...
public class DfmVisualisation {
	...
	private GoalDrivenDot dot;
	private ProcessTreeVisualisationInfo info;
	private TraceViewEventColourMap traceViewColourMap;

	private TIntObjectHashMap<GoalDrivenDotNode> node2input;
	private TIntObjectHashMap<GoalDrivenDotNode> node2output;

	public Triple<Dot, ProcessTreeVisualisationInfo, TraceViewEventColourMap> fancy(IvMModel model,
			AlignedLogVisualisationData data, ProcessTreeVisualisationParameters parameters, MapValueGroupObject mapValueGroup) throws IOException {
		...
		GoalDrivenDotNode source = new GoalDrivenDotNode(dot, info, NodeType.source, "", -1, null);
		node2output.put(-1, source);
		GoalDrivenDotNode sink = new GoalDrivenDotNode(dot, info, NodeType.sink, "", -1, source);
		...
		for (int activity : dfg.getNodeIndices()) {
			String actName = dfg.getNodeOfIndex(activity);
			Color c = Color.WHITE;
			for (AttributeClassifier att: mapValueGroup.getMapValueGroup().keySet()) {
				if (att.toString().equals(actName)) {
					c = mapValueGroup.getMapValueGroup().get(att).getGroupColor();
				}
			}
			Triple<String, Long, Long> cardinality = data.getNodeLabel(activity, false);
			convertActivity(model.getDfg(), activity, cardinality, c);
		}
	...
	private LocalDotNode convertActivity(DirectlyFollowsModel dfg, int msdNode,
			Triple<String, Long, Long> cardinality, Color c) {
		...
		final GoalDrivenDotNode dotNode = new GoalDrivenDotNode(dot, info, NodeType.activity, label, msdNode, null);
		...
		if (hasModelMoves) {
			...
			GoalDrivenDotNode before = new GoalDrivenDotNode(dot, info, NodeType.xor, "", -1, null);
			...
			GoalDrivenDotNode after = new GoalDrivenDotNode(dot, info, NodeType.xor, "", -1, null);
		...
		if (!c.equals(Color.WHITE)) {
			dotNode.setOption("color", String.format("#%02x%02x%02x", c.getRed(), c.getGreen(), c.getBlue()));
		}
