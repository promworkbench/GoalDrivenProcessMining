...
public class CustomEdgeStrokeColorAction extends ColorAction {
	private HashMap<EdgeObject, Color> mapEdgeColor;

	public CustomEdgeStrokeColorAction(String group,  HashMap<EdgeObject, Color> mapEdgeColor) {
		...
		if (item instanceof Edge) {
			...
			for (EdgeObject edgeObject : this.mapEdgeColor.keySet()) {
				if (edgeObject.getNode1().equals(source) && edgeObject.getNode2().equals(target)) {
					item.setInt(GraphConstants.EDGE_FILL_COLOR_FIELD, ColorLib.color(this.mapEdgeColor.get(edgeObject)));
